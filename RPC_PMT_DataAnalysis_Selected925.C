...
// 2026.05.11 优化 calculateSignalParams 函数, 提前筛选数据点，每一个点都采集。循环拟合2次。
...
const string selectedPngFolder = "/ustcfs/STCFUser/yzhao/LowPressure/ExpDatas_2606/40_50_10/3kpa/selected_waveforms_png/780";
const char* outputFileName = "590um_3kPa_40iso_selected780_from780";
...
vector<int> LoadSelectedWaveformIds(const string& pngFolder) {
    set<int> ids;
    try {
        for (const auto& entry : fs::directory_iterator(pngFolder)) {
            ...
            if (!all_of(stem.begin(), stem.end(), [](unsigned char c) { return isdigit(c); })) continue;

            ids.insert(stoi(stem));
        }
    } catch (const fs::filesystem_error& e) {
        cerr << "错误：无法读取选择波形PNG目录 - " << e.what() << endl;
    }

    vector<int> out(ids.begin(), ids.end());
...
SignalParams calculateSignalParams( const vector<double>& time,
    ...
    threshold_search_start_idx = max(1, threshold_search_start_idx);
    threshold_search_start_idx = min(threshold_search_start_idx, sig_size - 1);
...
void processVoltageSerial(int voltage, const string& folder_name,
    ...
    int csvCount = selected_ids.size();
    ...
    int missingCount = 0;
    for (size_t idx = 0; idx < selected_ids.size(); ++idx) {
        const int file_num = selected_ids[idx];
        ...
        if (!readCSVData(filename, rpc_time, pmt_time, rpc_signal, pmt_signal)) {
            ++missingCount;
            cout << "WARNING: selected file missing, skip " << filename << endl;
            continue;
        }
        ...
        if (processedCount % 10 == 0) {
            cout << "folder_name: " << folder_name << "V, processed selected signals: "
                 << processedCount << "/" << csvCount
                 << "  ****progress**** " << processedCount * 100 / max(1, csvCount) << "%" << endl;
        }
    }

    cout << "folder_name: " << folder_name << "V, selected IDs: " << csvCount
         << ", processed CSV: " << processedCount
         << ", missing CSV: " << missingCount << endl;
    ...
    for (int voltage : voltages) {
        ...
        for (int i = 0; i < data_size; ++i) {
            ...
            // if (RT_cut && FWHM_cut) {  // 优化：去掉幅度筛选，保留时间相关筛选
             if (FWHM_cut ) {  // 优化：去掉幅度筛选，保留时间相关筛选
        ...
        const double h2A_lo = max(1.0, meanA - sigma_left*rmsA);
...
void analyzeDataSerial() {
    ...
    vector<int> voltages = {780};
    vector<int> selected_ids = LoadSelectedWaveformIds(selectedPngFolder);
    cout << "Selected waveform PNG IDs loaded: " << selected_ids.size()
         << " from " << selectedPngFolder << endl;
    if (selected_ids.empty()) {
        cerr << "ERROR: no selected waveform IDs found. Stop analysis." << endl;
        return;
    }
    // // 打印结果
    // for (auto v : voltages) {
    ...
    for (int voltage : voltages) {
        ...
        const double A_bin_width = 0.2; // 幅值的bin宽度设置为2 mV
        const double Q_bin_width = 2e4; // 电荷的bin宽度设置为1e3 e
        const double hA_min = 0, hA_max = 200; // 幅值范围设置为0-1500 mV
...
void RPC_PMT_DataAnalysis_Selected925() {
